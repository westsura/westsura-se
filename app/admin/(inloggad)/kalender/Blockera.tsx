      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {blockeringar.map((b) => (
          <span key={b.id} className="pill pill--block" style={{ display: "inline-flex", gap: 8, alignItems: "center" }}>
            ...
            <button style={{ background: "none", border: 0, cursor: "pointer", color: "inherit", fontWeight: 700 }} title="Ta bort" disabled={pending} onClick={() => start(async () => { setFel(null); const r = await taBortBlockering(b.id); if (!r.ok) setFel(r.fel ?? "Kunde inte ta bort blockeringen."); })}>×</button>
          </span>
        ))}
      </div>
      {open && (
        <form className="form admin__panel" style={{ marginTop: 14 }} onSubmit={(e) => { e.preventDefault(); const f = e.currentTarget; const fd = new FormData(f); const valda = Array.from(f.querySelectorAll<HTMLInputElement>("input[name=enhet]:checked")).map((i) => i.value); fd.set("enheter", valda.join(",")); setFel(null); start(async () => { const r = await skapaBlockering(fd); if (r.ok) setOpen(false); else setFel(r.fel ?? "Kunde inte blockera."); }); }}>
