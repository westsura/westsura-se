...
export default function ManuellBokning({ enheter }: { enheter: { id: string; namn: string }[] }) {
  ...
  return (
    <div className="admin__panel" style={{ marginBottom: 24 }}>
      ...
      <form className="form" onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); const valda = Array.from(e.currentTarget.querySelectorAll<HTMLInputElement>("input[name=enhet]:checked")).map((i) => i.value); fd.set("enheter", valda.join(",")); setMsg(null); setFel(null); start(async () => { const r = await skapaManuellBokning(fd); if (r.ok) { setMsg(`Bokning ${r.nummer} inlagd och bekräftad.`); setOpen(false); } else setFel(r.fel || "Kunde inte lägga in bokningen."); }); }}>
