...
export default function Fakturafalt({ prefix = "fk", full = false }: { prefix?: string; full?: boolean }) {
  ...
  return (
    <div className={full ? "field--full" : undefined}>
      ...
      {open && (
        <div className={`form form--sub${full ? "" : " form--1"}`}>
          ...
          <div className="field field--full"><label htmlFor={`${prefix}-adress`}>Fakturaadress</label><textarea id={`${prefix}-adress`} name="faktura_adress" className="ta--xs" placeholder="Gata, postnummer och ort" /></div>
